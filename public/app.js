...
function renderAuth(status = '', statusClass = '') {
  ...
  authView.innerHTML = `
    <div class="auth-grid single">
      <div class="panel auth-single">
        <div class="auth-switch-wrap">
          <button id="authSwitch" class="btn secondary" type="button">${isLogin ? t('newRegister') : t('backToLogin')}</button>
        </div>
...
function avatarPixels(gender) {
  const hair = gender === 'female' ? '#ff5cae' : '#a0a0a0';
  const outfit = gender === 'female' ? '#ca3a80' : '#5b5b5b';
  return [
    [60, 0, hair],
    [80, 0, hair],
    [40, 20, hair],
